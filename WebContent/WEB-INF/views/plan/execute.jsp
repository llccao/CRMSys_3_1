...
<script type="text/javascript">
	$(function() {
		...
		$("#stop").click(function() {
			...
			if (confirm("确定终止" + lable + "的计划吗?")) {
				var url = "${ctp}/plan/stop/" + id;
...
<body class="main">
	...
	<div class="button_bar">
		...
		<button class="common_button" id="stop">终止开发</button>
		<input type="hidden" value="${chance.custName }"/>
		<button class="common_button"
			onclick="window.location.href='${ctp}/plan/make?id=${chance.id }'">制定计划</button>
		<button class="common_button" onclick="javascript:history.go(-1);">返回</button>
		<button class="common_button"
			onclick="window.location.href='${ctp}/chance/finish?id=${chance.id }'">开发成功</button>
	</div>
